...
Vue.component('brand-list', {
    data: function () {
      return {
        ...
        loading: true, //загружаются брэнды
        loadError: false //на старте не известно будет ли ошибка сервера
      }
    },
    methods: {
      getModels: function () {
    ...
    template: `
      <div>
          <select v-bind:class="{ loading: loading }" v-model="selected" v-on:change="getModels">

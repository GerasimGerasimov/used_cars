...
function getDebugPrice(){
    ...
    if (err) {
        return 'Автомобиль с указанными параметрами на данный момент отсутствует в нашей базе. Вы можете повторить запрос позже или связаться с нами для решения данной проблемы';
    }
    else {
